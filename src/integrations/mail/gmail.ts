...
function getGmailConfig(recipientEmail: string) {
...
function loadProjectEnv(): Record<string, string | undefined> {
...
function parseGmailAccounts(value: string | undefined): GmailAccountConfig[] {
  if (!value?.trim()) {
    throw new Error(
      'ENVIRONMENT_ERROR: Gmail IMAP is not configured. Set GMAIL_ACCOUNTS in the project env file before running mail-dependent stages.'
    );
  }
  ...
  try {
    ...
    throw new Error(
      `ENVIRONMENT_ERROR: GMAIL_ACCOUNTS must be a JSON array. ${error instanceof Error ? error.message : String(error)}`
    );
  }
...
export function selectGmailAccountForRecipient(
  recipientEmail: string,
  accounts: GmailAccountConfig[]
): GmailAccountConfig {
  const recipient = normalizeEmail(recipientEmail);
  const account = accounts.find(candidate =>
    [...candidate.match, candidate.user].some(pattern => matchesEmailPattern(pattern, recipient))
  );

  if (!account) {
    throw new Error(
      `ENVIRONMENT_ERROR: No Gmail account in GMAIL_ACCOUNTS matches recipient "${recipientEmail}". ` +
...
function normalizeGmailAccount(value: unknown, index: number): GmailAccountConfig {
...
function parseProjectEnv(content: string) {
  const env: Record<string, string> = {};
...
function parseProjectEnvLine(lines: string[], startIndex: number) {
...
function hasClosingQuote(value: string, quote: string) {
  return closingQuoteIndex(value, quote) >= 0;
}

function closingQuoteIndex(value: string, quote: string) {
...
function unescapeEnvValue(value: string, quote: string) {
  if (quote === "'") {
...
function stripEnvComment(value: string) {
  return value.replace(/\s+#.*$/, '');
}

function matchesEmailPattern(pattern: string, recipientEmail: string) {
...
export async function waitForMail<T>(
  ...
  let lastCandidateSummaries: MailCandidateDiagnostic[] = [];
  ...
  throw new Error(
    `TIMEOUT: Waited for ${options.describe} sent to ${options.recipientEmail}` +
...
export function summarizeMailCandidatesForDiagnostics(messages: ParsedInboxMessage[]): MailCandidateDiagnostic[] {
...
export function formatMailCandidateSummaries(candidates: MailCandidateDiagnostic[]) {
...
export function isRetryableMailReadError(error: unknown) {
  const message = describeError(error);
  if (/ENVIRONMENT_ERROR|GMAIL_ACCOUNTS|Gmail IMAP is not configured|No Gmail account/i.test(message)) {
    return false;
  }

  return /network|socket|TLS|timeout|ETIMEDOUT|ECONN|ECONNRESET|EPIPE|ENOTFOUND|imap|gmail|NoConnection|Connection not available/i.test(
    message
  );
}
...
function trimDiagnosticText(value: string, maxLength: number) {
  const normalized = value.replace(/\s+/g, ' ').trim();
  return normalized.length > maxLength ? `${normalized.slice(0, maxLength - 3)}...` : normalized;
}

async function readRecentInboxMessages(after: Date, recipientEmail: string): Promise<ParsedInboxMessage[]> {
...
async function readMailboxCandidates(client: ImapFlow) {
...
async function readRecentMailboxMessages(
  client: ImapFlow,
  mailbox: string,
  since: Date,
  after: Date
...
async function parseMessage(message: FetchMessageObject): Promise<ParsedInboxMessage> {
  if (!message.source) {
...
export function matchesRecipient(message: ParsedInboxMessage, recipientEmail: string) {
...
export function messageContains(message: ParsedInboxMessage, value: string) {
...
function addressList(value?: AddressObject | AddressObject[]) {
  ...
  return values.flatMap(address => address.value.map(item => item.address).filter((address): address is string => Boolean(address)));
}
...
export function normalizeMailText(value: string, options: { preserveLines?: boolean } = {}) {
...
export function htmlToText(html: string | false, options: { preserveLines?: boolean } = {}) {
  if (!html) {
    return '';
  }
